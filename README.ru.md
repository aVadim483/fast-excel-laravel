...
* Чтение
  * Читает XLSX, устаревший XLS (Excel 97-2003) и CSV; формат таблицы определяется автоматически по содержимому файла
  * Читает книгу из файла, из строки или из любого потока (диск Laravel, поле в базе, URL)
...
* [Импорт данных](#импорт-данных)
  ...
  * [Скорость импорта](#скорость-импорта)
  * [Расширенные возможности импорта](#расширенные-возможности-импорта)
  * [Чтение CSV-файлов](#чтение-csv-файлов)
  * [Чтение из строки или потока](#чтение-из-строки-или-потока)
...
```Excel::open()``` читает XLSX, устаревший XLS (Excel 97-2003) и CSV — формат определяется 
автоматически по содержимому файла, поэтому расширение не имеет значения. Все методы импорта ниже 
работают одинаково независимо от исходного формата (см. [Чтение устаревших XLS-файлов](/docs/80-reading-xls.md) 
и [Чтение CSV-файлов](/docs/81-reading-csv.md)).
...
`importModel()` импортирует весь лист в одной транзакции базы данных: база не фиксирует каждую строку
отдельно, поэтому импорт идёт намного быстрее, а если какая-то строка завершится ошибкой, не будет
импортировано ничего.

Для больших файлов строки можно вставлять пакетами: строки накапливаются и вставляются одним запросом на пакет
вместо отдельного `save()` на каждую строку

```php
// Insert rows in batches of 1000
$excel->withHeadings()->importModel(User::class, batchSize: 1000);
```
С `batchSize` модели не сохраняются по одной, поэтому события Eloquent (`creating`, `saved`, наблюдатели, …)
не срабатывают и модели не получают id; мутаторы и приведения типов атрибутов применяются, временные метки
проставляются. Не делайте пакет слишком большим: число плейсхолдеров в одном запросе ограничено базой
(65535 в MySQL, 32766 в SQLite), поэтому `batchSize × число колонок` должно оставаться меньше этого предела.
...
У CSV нет сигнатуры, поэтому ```Excel::open()``` считает CSV любой файл, который не является ни XLSX, ни XLS.
Весь API импорта — `withHeadings()`, `mapping()`, `importModel()`, области чтения, `readRows()`, `nextRow()` —
работает точно так же, как для XLSX и XLS.
...
Для чтения CSV не нужен отдельный пакет — ридер устанавливается вместе с этим. Глобальные значения
по умолчанию для CSV-опций можно задать в `config('fast-excel.csv')`. Учтите, что у CSV нет типов и стилей
ячеек, поэтому каждое значение читается как строка; запись CSV не поддерживается (экспорт — только XLSX).
См. [Чтение CSV-файлов](/docs/81-reading-csv.md).

### Чтение из строки или потока

Книга не обязана быть файлом на локальном диске. ```Excel::openString()``` читает её из строки,
```Excel::openStream()``` — из любого открытого потока (stream resource). Формат определяется по содержимому
ровно так же, поэтому работают и XLSX, и XLS, и CSV, а дальше доступен весь API импорта.
...
Оба метода принимают те же опции, что и ```Excel::open()```. Временная копия удаляется по завершении скрипта,
а поток не закрывается — им по-прежнему владеет вызывающий код.
См. [Чтение из строки или потока](/docs/82-reading-from-memory.md).
